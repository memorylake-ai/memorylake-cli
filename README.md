...
Prefer not to pipe a script into your shell? Every release publishes
per-platform archives with SHA-256 checksums on the
[releases page](https://github.com/memorylake-ai/memorylake-cli/releases) —
download one, check it against its `.sha256`, and put `memorylake` on your
`PATH`. Then run `auth login` and `workspace use` yourself: the installers do
nothing the CLI cannot do on its own.
